...
public class TestToken {
	...
	public void testConstructorAndAccessors() {
		Token token = new Token(WaebricLiteral.MODULE, TokenSort.LITERAL, 123);
		assertNotNull(token);
		assertTrue(token.getLexeme() == WaebricLiteral.MODULE);
		assertTrue(token.getSort() == TokenSort.LITERAL);
		assertTrue(token.getLine() == 123);
	}
	
	@Test
	public void testEquals() {
		Token module = new Token(WaebricLiteral.MODULE, TokenSort.LITERAL, 123);
		Token similar = new Token(WaebricLiteral.MODULE, TokenSort.LITERAL, 123);
		Token difflit = new Token(WaebricLiteral.END, TokenSort.LITERAL, 123);
		Token diffsort = new Token(WaebricLiteral.MODULE, TokenSort.IDCON, 123);
		Token diffline = new Token(WaebricLiteral.MODULE, TokenSort.LITERAL, 124);
